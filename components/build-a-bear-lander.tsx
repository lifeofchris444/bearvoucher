...
const AFFILIATE_LINK = "https://trksy.org/aff_c?offer_id=4664&aff_id=26188"

const BEAR = "#0057B8"
const BEAR_DEEP = "#04121F"
const BEAR_BRIGHT = "#4FA8F5"
const INK = "#111213"
...
const BEAR_ICON =
  "M12 21a6.2 6.2 0 100-12.4 6.2 6.2 0 000 12.4 M7.2 8.1a2.6 2.6 0 113.3-3.6 M16.8 8.1a2.6 2.6 0 10-3.3-3.6 M10.2 13.6h.01 M13.8 13.6h.01 M10.4 16.8c.7.6 2.5.6 3.2 0"

const OUTFIT_ICON = "M8.4 4L12 6l3.6-2L20 7.2l-2.2 3.2-1.3-.9V20.5H7.5V9.5l-1.3.9L4 7.2z"

const HEART_ICON =
  "M12 20.6c-4.1-3-7.1-5.5-7.1-9.1A3.9 3.9 0 0112 7.9a3.9 3.9 0 017.1 3.6c0 3.6-3 6.1-7.1 9.1z M3 5.5c.9-.9 1.8-.9 2.7 0 M21 5.5c-.9-.9-1.8-.9-2.7 0"

const GIFT_ICON =
  "M3.4 8.2h17.2v4H3.4z M4.8 12.2h14.4v8.6H4.8z M12 8.2v12.6 M8.6 8.2a2.6 2.6 0 010-5.2c2.1 0 3.4 2.6 3.4 5.2 M15.4 8.2a2.6 2.6 0 000-5.2C13.3 3 12 5.6 12 8.2"

const CATEGORIES = [
  {
    t: "Furry Friends & Bears",
    d: "Classic teddies, seasonal plush, and collectible characters.",
    icon: BEAR_ICON,
  },
  {
    t: "Outfits & Costumes",
    d: "Tees, dresses, pajamas, and tiny shoes for every furry friend.",
    icon: OUTFIT_ICON,
  },
  {
    t: "Sounds & Scents",
    d: "Recorded messages, beating hearts, and scented heart inserts.",
    icon: HEART_ICON,
  },
  {
    t: "Accessories & Gift Sets",
    d: "Wristlets, sleeping bags, and ready-to-give bundle boxes.",
    icon: GIFT_ICON,
  },
]
...
const AVATARS = [
  { src: "/avatars/bab-shopper-1.png", alt: "Smiling mom with shoulder-length brown hair in a denim shirt" },
  { src: "/avatars/bab-shopper-2.png", alt: "Smiling mom with dark hair pulled back and glasses in a green blouse" },
  { src: "/avatars/bab-shopper-3.png", alt: "Smiling young woman with braided hair in a mustard cardigan" },
]

function Icon({ d, className = "w-6 h-6", color = BEAR }: { d: string; className?: string; color?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      className={className}
...
function CTAButton({
  ...
  return (
    <a
      ...
      style={dark ? { backgroundColor: BEAR, color: "#FFFFFF" } : { backgroundColor: "#FFFFFF", color: INK }}
    >
      {children}
      <svg
        viewBox="0 0 24 24"
  ...
  return (
    <div className="relative grid grid-cols-[3rem_1fr] md:grid-cols-[1fr_3.5rem_1fr] gap-x-4 md:gap-x-8 items-center pb-6 md:pb-8 last:pb-0">
      <div
        ...
        aria-hidden="true"
      >
        <span className="font-black text-6xl lg:text-7xl leading-none select-none" style={{ color: "#D3E6FA" }}>
          0{index + 1}
        </span>
      </div>
      ...
      <div className="order-1 md:order-2 flex justify-center self-start md:self-center pt-1 md:pt-0">
        <div
          ...
          style={{
            ...
            borderColor: BEAR,
            color: "#FFFFFF",
            boxShadow: "0 8px 22px rgba(0,87,184,.34)",
          }}
        >
          {index + 1}
        </div>
      ...
      >
        <div className={`flex items-start gap-4 ${leftSide ? "md:flex-row-reverse md:text-right" : ""}`}>
          <div
            ...
            style={{ backgroundColor: "#E4F0FC" }}
          >
            <Icon d={step.icon} color={BEAR} className="w-6 h-6" />
          </div>
...
export default function BuildABearRewardLander() {
  ...
  return (
    <div className="min-h-screen bg-white antialiased overflow-x-hidden font-sans" style={{ color: INK }}>
      <style>{`
        ...
        .card-shine { position: absolute; inset: 0; pointer-events: none; background: linear-gradient(115deg, transparent 35%, rgba(0,87,184,0.06) 47%, rgba(255,255,255,0.55) 52%, transparent 65%); background-size: 260% 100%; animation: cardShine 7s ease-in-out infinite; }
        ...
        .cat-spark { position: absolute; top: 16px; right: 18px; color: #0057B8; animation: sparkTwinkle 3.4s ease-in-out infinite; }
        ...
        .t-card:hover { transform: translateY(-6px); border-color: rgba(0,87,184,.4); box-shadow: 0 18px 44px rgba(0,87,184,.14); }
          .t-rail-fill { background: linear-gradient(180deg, #4FA8F5, #0057B8); }
      ...
      <section
        ...
        style={{ background: `linear-gradient(170deg, ${BEAR_BRIGHT} 0%, ${BEAR} 45%, ${BEAR_DEEP} 100%)` }}
      >
        <div className="orb w-96 h-96 -top-24 -left-24" style={{ backgroundColor: "#8FC6F7" }} />
        <div className="orb w-80 h-80 top-1/3 -right-20" style={{ backgroundColor: "#04121F", animationDelay: "-6s" }} />
        <div className="drifter left-[8%] top-[30%]">
          <Icon d={BEAR_ICON} color="#fff" className="w-9 h-9" />
        </div>
        <div className="drifter right-[10%] top-[18%]" style={{ animationDelay: "-3s" }}>
          <Icon d={HEART_ICON} color="#fff" className="w-8 h-8" />
        </div>
        <div className="drifter left-[16%] bottom-[12%]" style={{ animationDelay: "-5s" }}>
          <Icon d={GIFT_ICON} color="#fff" className="w-8 h-8" />
        </div>
        ...
        <div className="relative mx-auto max-w-3xl px-6 pt-12 pb-20 md:pt-20 md:pb-24 text-center">
          <div className="text-white">
            ...
            <h1 className="mt-6 font-black tracking-tight leading-[1.05] text-[1.64rem] min-[344px]:text-[1.82rem] min-[376px]:text-[2rem] min-[408px]:text-[2.18rem] min-[440px]:text-[2.36rem] min-[472px]:text-[2.54rem] min-[504px]:text-[2.71rem] min-[536px]:text-[2.89rem] min-[568px]:text-[3.07rem] min-[600px]:text-[3.25rem] min-[632px]:text-[3.43rem] min-[664px]:text-[3.61rem] min-[696px]:text-[3.78rem] min-[728px]:text-[4.05rem] text-balance">
              {"The Build-A-Bear Discounts They Don't Advertise"}
            </h1>
            <p className="mt-6 text-lg md:text-xl text-white/85 max-w-xl mx-auto leading-relaxed text-pretty">
              Discover how shoppers are unlocking hidden discount codes on furry friends, outfits, sounds and scents, and
              gift sets.
            </p>
      ...
      <section className="relative overflow-hidden bg-white">
        <div className="relative mx-auto max-w-5xl px-6 py-16 md:py-20">
          <div className="text-center">
            <div className="text-sm font-semibold tracking-widest uppercase" style={{ color: BEAR }}>
              How it works
            </div>
              ...
              <div key={f.q} className="rounded-2xl bg-white border border-gray-200 shadow-sm">
                ...
                >
                  ...
                  <span
                    ...
                    style={{ backgroundColor: openFaq === i ? BEAR : "#F2F3F5" }}
                  >
                    <svg
                      viewBox="0 0 20 20"
      ...
      <section
        className="relative overflow-hidden"
        style={{ background: `linear-gradient(170deg, ${BEAR_BRIGHT} 0%, ${BEAR} 50%, ${BEAR_DEEP} 100%)` }}
      >
        {[
          { l: "6%", s: 5, d: 7, delay: 0 },
        ...
        <div className="orb w-96 h-96 -top-24 -left-24" style={{ backgroundColor: "#A9D5F9", opacity: 0.35 }} />
        <div
          className="orb w-80 h-80 -bottom-24 -right-16"
          style={{ backgroundColor: "#04121F", animationDelay: "-5s" }}
        />
